...
public class SetTest {
	 ...
	 */
     ...
     //Set의 데이터 수정하기   list.set(idnex,새로운데이터)
     // => set에는 데이터를 수정하는 메서드가 따로 없다.
     //그래서 수정하는 방법은 해당 자료를 삭제한 후에 추가해 주는 방법을 사용한다.
...
//	 list.get(index)
     ...
     /*
      ...
      그래서 데이터를 하나씩 차레로 얻기 위해서는 Iterator형 객체로 변환해야 한다.
   ...
   System.out.println("-----------------------------------------------");
	...
	// 우리반 학생들 중 번호를 이용하여 추첨하는 프로그럄을 작성해 보자.
	// 번호는 1번부터 28번까지 있고, 추첨할 인원이 3명이다. 
	// 당첨번호를 출력하시오. 
	
	// 36~ 77
	// rnd.nextInt(최대값 - 최솟값 +1) +최솟값
